...
def get_retaining_wall_price_data():
    unit_price_data = {
...
def get_cost_data():
    unit_price_data = {
        '項目': ['擋土工程','渠道工程','版橋工程','道路工程','安全設施'],
        '總價': [st.session_state.retaining_wall_cost,st.session_state.open_channel_cost,st.session_state.bridge_cost,st.session_state.road_cost,0],  
        '單位': ['元']*5
    }
...
def main():
    ...
    if 'retaining_wall_cost' not in st.session_state:
        st.session_state.retaining_wall_cost = 0
    ...
    with st.sidebar:
        ...
        with st.expander(":world_map: 擋土設施基本單價表"):
            edited_retaining_wall_price_df = st.data_editor(get_retaining_wall_price_data(), hide_index=True)
    # Tab
    ...
    with tabs[0]:
        render_retaining_wall_tab(edited_retaining_wall_price_df)
    with tabs[1]:
    ...
    with st.sidebar:
        ...
        coe=st.number_input("間接費用係數",min_value=0.0,value=0.3,step=0.05)
